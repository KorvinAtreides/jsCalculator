...
const allCalculation = {
  '+': (firstOperand, secondOperand) => truncated((firstOperand + secondOperand),Math.max(digitsCount(firstOperand),digitsCount(secondOperand))),
  ...
  '^': (firstOperand, secondOperand) => truncated((Math.pow(firstOperand, secondOperand)),Math.max(digitsCount(firstOperand),digitsCount(secondOperand))),
  'sin': (firstOperand, secondOperand) => truncated(Math.sin(firstOperand*Number(val))),
  'cos': (firstOperand, secondOperand) => truncated(Math.cos(firstOperand*Number(val))),
  'tg': (firstOperand, secondOperand) => truncated(Math.tan(firstOperand*Number(val))),
  'ctg': (firstOperand, secondOperand) => truncated(1 / Math.tan(firstOperand*Number(val))),
...
function inputOperand(ourOperand) {
  ...
  if (waitingForSecondOperand === true) { //ввод второго числа
      calculator.displayValue = ourOperand;
      calculator.waitingForSecondOperand = false;
  } else {
      calculator.displayValue = displayValue === '0' ? ourOperand : displayValue + ourOperand; //всё ещё вводится первое число
  }
  //console.log(calculator); 
}

function truncated(num, decimalPlaces = 7) {    //округление - умножаем на 10 в степени кол-ва значащих цифр или 7
...
function digitsCount(str) { 
  ...
  else{return (String(str).length-pointPos-1)
  }
}
...
function handleOperator(nextOperator) {
  ...
  if (firstOperand == null) {
      ...
      if (operator == '/' && inputValue == 0) {
        ...
        document.getElementById("ulOfOperations").innerHTML+=`<li>Error: Division by 0</li>`;
      } else if(operator == '^' && inputValue == 0 && currentValue == 0){
        const result = NaN;
        calculator.displayValue = String(result);
        calculator.firstOperand = result;
        document.getElementById("ulOfOperations").innerHTML+=`<li>Error: Indetermination </li>`;
      }
...
function historyAdd(currentValue1, operator1, inputValue1, result1){
  ...
  if(operator1=="n!"){
    operator1="!";
    ul.innerHTML+=`<li>`+currentValue1+ operator1+ ` = `+result1+`</li>`;
  }
  else if(operator1=="sin"||operator1=="cos"||operator1=="tg"||operator1=="ctg"){
    if(val==1){
